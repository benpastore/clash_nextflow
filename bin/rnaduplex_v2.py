...
# -------------------------------
# Bulge detection function (unchanged)
# -------------------------------
def detect_bulges(dot_bracket):
    """
    Finds true bulges in a dot-bracket structure, distinguishing them from fully unpaired regions.
...
# -------------------------------
# RNAduplex function with full-length padding for both target and smRNA
# -------------------------------
def run_rnaduplex(seq_pair):
    ...
    The final duplex structure is modified so that both the target and smRNA portions
    are padded with dots to match the full lengths of the extended target and corrected smRNA.
    """
    try:
        ...
        if len(output_lines) == 1:
            ...
            # Typically, RNAduplex outputs two lines: header and then structure + energy.
            header_line = output_lines[0].strip()  # Not used in this version.
        ...
        # Reassemble the final duplex structure.
...
def is_tail_base_paired(row):
    ...
    """
    tail_seq = row["tail"]  # e.g. "U", "AA", or "*" for no tail
