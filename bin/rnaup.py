...
def detect_bulges(dot_bracket):
    """Finds true bulges in dot-bracket notation, distinguishing them from single-stranded loops."""
    if "&" not in dot_bracket:
        return "Invalid Structure", "Invalid Structure"
    ...
    try:
        ...
        # Function to find true bulges
        def find_true_bulges(structure, complementary_structure, offset=0):
            bulge_positions = []
            
            for match in re.finditer(r"\.+", structure):  # Find unpaired stretches
                start, end = match.start(), match.end() - 1  # Get start and end positions
                
                # Ensure indices are within valid range
                if end >= min_length:
                    continue  # Skip if beyond valid length

                # Count paired bases in the complementary strand
                paired_bases = sum(1 for i in range(start, end + 1) if i < min_length and complementary_structure[i] in "()")

                # A true bulge has unpaired bases in one strand, but paired bases in the other
                if paired_bases > 0:
